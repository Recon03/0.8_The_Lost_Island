...
public class MotionControllerEditor : Editor
{
    ...
    private List<string> mGroupNames = new List<string>();

    // Determines if we show the layers and motions
    //private bool mShowLayers = false;
    private bool mShowSettings = false;

    // Defines how we'll clear the animator parameters
    private string[] mAnimatorClearTypes = new string[] { "Clear normally", "Clear after delay" };
            ...
            if (typeof(MotionControllerMotion).IsAssignableFrom(lType))
            {
                ...
                if (lStaticMethods != null)
                {
                    for (int j = 0; j < lStaticMethods.Length; j++)
                    {
                        ...
                        else if (lStaticMethods[j].Name == "GroupName")
                        {
                            string lGroupName = lStaticMethods[j].Invoke(null, null) as string;
                            if (mGroupNames.IndexOf(lGroupName) < 0)
                            {
                                mGroupNames.Add(lGroupName);
                            }
                        }
                    }
                }
    ...
    private bool OnAdvancedInspector()
    {
        ...
        if (mLayerList.index >= 0 && mLayerList.index < mTarget.MotionLayers.Count)
        {
            ...
            int lNewEditorFilterGroupIndex = EditorGUILayout.Popup(mTarget.EditorFilterGroupIndex, mGroupNames.ToArray(), GUILayout.Width(100));
    ...
    private void InstanciateMotionList(ReorderableList rLayerList)
    {
        if (IsFiltering)
        {
            ...
            string lGroupText = mGroupNames[mTarget.EditorFilterGroupIndex].ToLower();
            ...
            for (int i = 0; i < mTarget.MotionLayers[rLayerList.index].Motions.Count; i++)
            {
                ...
                if (mTarget.EditorFilterGroupIndex > 0 && lMotion._Pack.ToLower() != lGroupText) { continue; }
