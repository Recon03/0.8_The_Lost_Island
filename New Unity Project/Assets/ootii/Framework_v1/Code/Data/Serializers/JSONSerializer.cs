...
namespace com.ootii.Data.Serializers
{
    ...
    public class JSONSerializer
    {
        ...
        public static object Deserialize(string rJSON)
        {
            ...
            if (lType == null || lType.Length == 0) { return null; }

            object lObject = null;
            try
            {
                lObject = Activator.CreateInstance(Type.GetType(lType));
            }
            catch (Exception lException)
            {
                Log.ConsoleWriteError(String.Format("JSONSerializer.Deserialize() {0} {1} {2}", lType, lException.Message, lException.StackTrace));
            }
