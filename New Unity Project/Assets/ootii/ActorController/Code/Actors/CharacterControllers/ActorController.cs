...
namespace com.ootii.Actors
{
    ...
    public class ActorController : BaseSystemController, ICharacterController
    {
        ...
        /// </summary>
//        public void UpdateMovement_old(float rDeltaTime, int rUpdateIndex, Quaternion rFrameYaw, Quaternion rFrameTilt)
//        {
//            Vector3 lActorUp = _Transform.up;
//            Vector3 lActorPosition = _Transform.position;
...
//                        // TRT 12/04/16 - Added this logic to stop sliding
//                        if (!_OrientToGround || mTargetGroundNormal.sqrMagnitude == 0f)
//                        {
...
//                // TRT 12/31/2016: Only slide if gravity is enabled
//                // TRT 03/11/2017: Force more sliding when on something that exceeds the max angle
//                if (_IsGravityEnabled)
//                {
...
//                        lHitAngle > (_MaxSlopeAngle > 0f ? _MaxSlopeAngle - 0.5f : MAX_GROUNDING_ANGLE) && lHitAngle < 90f - EPSILON)
//                    {
//                        if (lIsGrounded)
//                        {
...
//        }
        ...
        public void UpdateMovement(float rDeltaTime, int rUpdateIndex, Quaternion rFrameYaw, Quaternion rFrameTilt)
        {
            ...
            while (true)
            {
                ...
                if (_IsCollisionEnabled)
                {
                    ...
                    if (lRemainingMovement.sqrMagnitude > EPSILON_SQR)
                    {
                        ...
                        if (Vector3.Dot(lHitDirection, lActorUp) < 0f)
                        {
            ...
            mState.Rotation = lFinalRotation;
            mState.Position = lFinalPosition;
            mState.Velocity = (mState.Position - mPrevState.Position) / (rDeltaTime > 0f ? rDeltaTime : Time.fixedDeltaTime);
        ...
        protected bool ProcessGrounding(Vector3 rActorPosition, Vector3 rOffset, Vector3 rActorUp, Vector3 rWorldUp, float rGroundRadius, out RaycastHit rGroundHitInfo)
            ...
            if (lIsGrounded)
                ...
                float lSkinWidth = (_UseTransformPosition ? _AltSkinWidth : _SkinWidth);
        ...
        //protected float GetGroundDistance(Vector3 rPosition)
        //{
        //    RaycastHit lHitInfo;
        //    Vector3 lRayStart = rPosition + (Vector3.up * _GroundingStartOffset);
        ...
        //        mState.GroundSurfaceAngle = Vector3.Angle(lHitInfo.normal, Vector3.up);
        //    }
        ...
        protected float GetGroundDistance(Vector3 rPosition, Vector3 rActorUp)
        {
            ...
            if (lIsHit)
                ...
                // TRT 12/11/16 - Modify the skin width for off-mesh links. Links are about 0.05f above the surface.
                float lSkinWidth = (_UseTransformPosition ? _AltSkinWidth : lGroundingDistance);
